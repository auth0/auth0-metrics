!function(){
  ...
  function loader(config) {
    ...
    if(null != window.Auth0Metrics){
      window.metricsLib = new Auth0Metrics(config.segmentKey, config.dwhEndpoint, config.label, config.options);
      if (config.onReady) window.metricsLib.ready(config.onReady);
      return;
    }
    ...
    script.src = ('https:' === document.location.protocol
      ? 'https://' : 'http://')
      + 'cdn.auth0.com/js/m/2.0.0/auth0-metrics.min.js';
